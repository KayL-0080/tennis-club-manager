...
export function renderCardToCanvas(canvas, type, data, options = {}) {
  ...
  roundRect(ctx, padX, curY, 360 * scale, 48 * scale, 24 * scale);
  ctx.fill();
  ...
  ctx.font = `bold ${18 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", "Segoe UI", Roboto, sans-serif`;
  ctx.textAlign = 'left';
  ctx.fillText(`${modeInfo.icon} ${clubName} • ${modeInfo.tag}`, padX + 20 * scale, curY + 31 * scale);
  ctx.restore();

  curY += 68 * scale;

  // 4. 메인 타이틀 (Hero Title) - 시인성 강화
  const mainTitle = data.title?.trim() || modeInfo.titleDefault;
  ctx.save();
  ctx.fillStyle = '#ffffff';
  ctx.font = `900 ${52 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", "Noto Sans KR", sans-serif`;
  ctx.textAlign = 'left';
  ctx.shadowColor = 'rgba(0, 0, 0, 0.5)';
  ctx.shadowBlur = 16 * scale;
  ctx.shadowOffsetY = 4 * scale;
  curY = drawWrappedText(ctx, mainTitle, padX, curY + 44 * scale, width - padX * 2, 62 * scale, 2);
  ctx.restore();
  ...
  const drawInfoItem = (icon, label, value, subValue = null, highlightColor = null) => {
    ...
    roundRect(ctx, badgeX, badgeY, badgeSize, badgeSize, 14 * scale);
    ctx.fill();

    ctx.font = `${26 * scale}px -apple-system, sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(icon, badgeX + badgeSize / 2, badgeY + badgeSize / 2 + 1);

    // 라벨 (글자 크기 15px -> 17px로 확대)
    const textX = badgeX + badgeSize + 18 * scale;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';
    ctx.fillStyle = 'rgba(255, 255, 255, 0.70)';
    ctx.font = `600 ${17 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
    ctx.fillText(label, textX, curY + 31 * scale);

    // 값 (글자 크기 20/22px -> 24/26px로 확대)
    ctx.fillStyle = highlightColor || '#ffffff';
    ctx.font = `800 ${subValue ? 24 * scale : 26 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
    ctx.fillText(value, textX, curY + (subValue ? 59 * scale : 62 * scale));

    if (subValue) {
      ctx.fillStyle = 'rgba(255, 255, 255, 0.80)';
      ctx.font = `500 ${16 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
      ctx.fillText(subValue, textX, curY + 84 * scale);
    }
  ...
  if (data.notes && data.notes.trim()) {
    ...
    ctx.font = `700 ${17 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
    ctx.textAlign = 'left';
    ctx.fillText('💡 안내 / 비고:', padX + 20 * scale, curY + 30 * scale);

    ctx.fillStyle = 'rgba(255, 255, 255, 0.90)';
    ctx.font = `500 ${16 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
    drawWrappedText(ctx, data.notes.trim(), padX + 20 * scale, curY + 54 * scale, contentW - 40 * scale, 22 * scale, 1);
    ctx.restore();
    curY += noteH + 16 * scale;
  } else {
    curY += 10 * scale;
  ...
  // 7. 하단 연락처 & 카톡ID 문의 바
  const contactH = 104 * scale;
  const contactY = height - padX - contactH;

  ctx.save();
  // 하단 프리미엄 컨택트 카드
  const contactGrad = ctx.createLinearGradient(padX, contactY, padX + contentW, contactY);
  contactGrad.addColorStop(0, 'rgba(30, 41, 59, 0.96)');
  contactGrad.addColorStop(1, 'rgba(15, 23, 42, 0.96)');
  ...
  // 상단 레이블 (14px -> 16px)
  ctx.fillStyle = modeInfo.theme;
  ctx.font = `bold ${16 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
  ctx.textAlign = 'left';
  ctx.fillText('💬 신청 및 문의하기', padX + 24 * scale, contactY + 34 * scale);

  // 연락처와 카톡 ID 한 줄 또는 두 열로 표시 (20px -> 24px)
  const phone = data.contact?.trim() || '운영진 문의';
  const kakao = data.kakaoId?.trim() || '';

  ctx.fillStyle = '#ffffff';
  ctx.font = `800 ${24 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
  let contactText = `📞 ${phone}`;
  if (kakao) {
    contactText += `   |   💬 카톡: ${kakao}`;
  }
  ctx.fillText(contactText, padX + 24 * scale, contactY + 74 * scale);

  // 우측 바로가기 배지 (14px -> 16px)
  ctx.fillStyle = modeInfo.theme;
  ctx.textAlign = 'right';
  ctx.font = `bold ${16 * scale}px -apple-system, BlinkMacSystemFont, "Pretendard", sans-serif`;
  ctx.fillText('빠른 문의 환영 →', padX + contentW - 24 * scale, contactY + 54 * scale);
