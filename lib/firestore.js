...
export async function initDefaultMembers(clubId) {
  if (!clubId) return;
  const snap = await getDocs(membersRef(clubId));
  if (!snap.empty) return; // 이미 존재
  const defaults = [
    { name: '이경열', role: '정회원', gender: 'M', ntrp: 3.5 },
    { name: '윤필용', role: '정회원', gender: 'M', ntrp: 3.0 },
    { name: '백성렬', role: '정회원', gender: 'M', ntrp: 3.0 },
    { name: '김응답', role: '정회원', gender: 'M', ntrp: 2.5 },
    { name: '강택수', role: '정회원', gender: 'M', ntrp: 1.5 },
    { name: '김신재', role: '정회원', gender: 'M', ntrp: 1.5 },
    { name: '이흥범', role: '정회원', gender: 'M', ntrp: 1.5 },
    { name: '이규현', role: '정회원', gender: 'M', ntrp: 1.5 },
    { name: '최민웅', role: '정회원', gender: 'M', ntrp: 2.5 },
    { name: '최옥경', role: '정회원', gender: 'F', ntrp: 2.0 },
    { name: '이가영', role: '정회원', gender: 'F', ntrp: 1.0 },
  ];
  for (const m of defaults) {
    await addDoc(membersRef(clubId), { ...m, createdAt: serverTimestamp() });
  }
}
