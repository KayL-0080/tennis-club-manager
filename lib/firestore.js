// lib/firestore.js — 확장판 (v3: members 컬렉션, schedule/scores/history 지원)
import {
...
export async function getClubMeetingSettings() {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'meetings');
  const snap = await getDoc(ref);
  if (!snap.exists()) return null;
  return snap.data();
}

export async function updateClubMeetingSettings(data) {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'meetings');
  await setDoc(ref, data, { merge: true });
}
