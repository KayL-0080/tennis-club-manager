...
export async function getMeetingRules() {
  ...
  if (!snap.exists() || !snap.data().rules || snap.data().rules.length === 0) {
    // 기본 디폴트 규칙 (일요일)
    return [
      { id: 'rule_sun', day: 0, dayName: '일요일', title: '정기모임', startTime: '12:00', endTime: '16:00', location: '금문테니스장 화전점', enabled: true }
    ];
  }
  const rules = snap.data().rules.map(r => ({
    ...r,
    title: r.title ? r.title.replace(/\s*\([월화수목금토일](?:요일)?\)/g, '').replace(/^정기\s*모임$/, '정기모임').trim() : '정기모임'
  }));
  return rules;
}
...
/* ══════════════════════════════════════════════════════════
   동호회 회칙 및 개정 이력 (bylaws) CRUD
══════════════════════════════════════════════════════════ */
export async function getClubBylaws() {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'bylaws');
  const snap = await getDoc(ref);
  if (!snap.exists()) return null;
  return snap.data();
}

export async function updateClubBylaws(data) {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'bylaws');
  await setDoc(ref, {
    ...data,
...
/* ══════════════════════════════════════════════════════════
   승점 및 순위 산정 기준 (rankingRules) CRUD
══════════════════════════════════════════════════════════ */
export async function getRankingRules() {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'rankingRules');
  const snap = await getDoc(ref);
  if (!snap.exists()) {
    return {
      winPoints: 3.0,
      ...
      bonusPerDay: 1.0,
      bonusPerMatch: 0.0,
      calcType: 'avg' // 'avg' (평균) | 'sum' (누적합)
    };
  }
  const d = snap.data();
  return {
    winPoints: d.winPoints !== undefined ? Number(d.winPoints) : 3.0,
    drawPoints: d.drawPoints !== undefined ? Number(d.drawPoints) : 2.0,
    lossPoints: d.lossPoints !== undefined ? Number(d.lossPoints) : 1.0,
    bonusPerDay: d.bonusPerDay !== undefined ? Number(d.bonusPerDay) : 1.0,
    bonusPerMatch: d.bonusPerMatch !== undefined ? Number(d.bonusPerMatch) : 0.0,
    calcType: d.calcType || 'avg'
  };
}

export async function updateRankingRules(data) {
  const ref = doc(db, 'shared', 'tcc-club', 'settings', 'rankingRules');
...
export async function getEvents(uid) {
  ...
  return snap.docs.map(d => {
    ...
    if (typeof data.attendees === 'string') {
      try { data.attendees = JSON.parse(data.attendees); } catch {}
    }
    if (typeof data.attendanceTimestamps === 'string') {
      try { data.attendanceTimestamps = JSON.parse(data.attendanceTimestamps); } catch {}
    }
    return { id: d.id, ...data };
  });
}
...
export async function getEvent(uid, eventId) {
  const ref = doc(db, 'shared', 'tcc-club', 'events', eventId);
  const snap = await getDoc(ref);
  if (!snap.exists()) return null;
  const data = snap.data();
  if (typeof data.attendees === 'string') {
...
/* 개별 회원의 투표 상태 및 수정 일시를 원자적(Atomic)으로 갱신하여 다른 회원의 투표 덮어쓰기 유실 방지 */
export async function updateEventMemberAttendance(uid, eventId, memberId, status, timestamp) {
  const ref = doc(db, 'shared', 'tcc-club', 'events', eventId);
