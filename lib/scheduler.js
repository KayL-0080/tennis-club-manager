...
export function computeGlobalStandings(schedules = [], members = [], startDate = '', endDate = '', tournaments = [], filterSource = 'ALL') {
  const stats = {};
  const attendedDates = {};
  const byId = {};
  members.forEach(m => {
  ...
  const rows = Object.values(stats).map(s => {
    const totalPoints = (s.win * 3) + (s.draw * 2) + (s.loss * 1);
    const avgPoints = s.played > 0 ? totalPoints / s.played : 0;
    const finalScore = avgPoints + (s.attendedDays * 1);
    return { 
      ...s, 
