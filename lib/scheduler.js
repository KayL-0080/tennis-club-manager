...
export function computeGlobalStandings(schedules, members, startDate, endDate) {
  ...
  const rows = Object.values(stats).map(s => {
    const totalEarnedPoints = s.win * 3 + s.draw * 2 + s.loss * 1;
    const avgPoints = s.played > 0 ? totalEarnedPoints / s.played : 0;
    const bonusPoints = s.attendedDays * 0.5;
    const totalPoints = Number((avgPoints + bonusPoints).toFixed(2));
