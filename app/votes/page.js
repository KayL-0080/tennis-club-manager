...
import { 
  ...
  getClubMeetingSettings,
  updateClubMeetingSettings
} from '@/lib/firestore';
...
export default function VotesPage() {
  ...
  const [clubMeetings, setClubMeetings] = useState([
    { dayOfWeek: 2, title: '정기 모임 (화)', startTime: '19:00', endTime: '22:00', location: '테니스장' },
    { dayOfWeek: 4, title: '정기 모임 (목)', startTime: '19:00', endTime: '22:00', location: '테니스장' }
  ]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const meetingData = await getClubMeetingSettings();
      const currentMeetings = meetingData?.regularMeetings || [
        { dayOfWeek: 2, title: '정기 모임 (화)', startTime: '19:00', endTime: '22:00', location: '테니스장' },
        { dayOfWeek: 4, title: '정기 모임 (목)', startTime: '19:00', endTime: '22:00', location: '테니스장' }
      ];
      setClubMeetings(currentMeetings);
      ...
      // Auto-generate missing events for the next 8 weeks (56 days)
      const now = new Date();
      const generated = [];
      if (currentMeetings.length > 0) {
        for (let i = 0; i < 56; i++) {
          ...
          const match = currentMeetings.find(m => m.dayOfWeek === day);
          if (match) {
            const dateStr = d.toLocaleDateString('en-CA'); // YYYY-MM-DD
            if (!evts.find(e => e.date === dateStr)) {
              const newEvent = {
                date: dateStr,
                title: match.title || `정기 모임 (${DAYS[day]})`,
                startTime: match.startTime || '19:00',
                endTime: match.endTime || '22:00',
                location: match.location || '테니스장',
                attendees: {}
              };
  ...
  const handleToggleAttendance = async (memberId, status) => {
    ...
    await updateEventAttendees('shared', selectedEvent.id, newAttendees);
    await updateEvent('shared', selectedEvent.id, { voteChanges: newVoteChanges });
  };
  ...
  const createCustomEvent = async () => {
    ...
    await createEvent('shared', newEvent);
  ...
  const removeEvent = async () => {
    ...
    await updateEvent('shared', selectedEvent.id, { isCancelled: true });
  ...
  const openSettingsModal = () => {
    setTempMeetings(JSON.parse(JSON.stringify(clubMeetings)));
    setShowSettingsModal(true);
  };

  const saveSettings = async () => {
    await updateClubMeetingSettings({ regularMeetings: tempMeetings });
  ...
  return (
    <div className={styles.page}>
      ...
      {/* 1. 정기 모임 요일 설정 모달 (Clean Standalone) */}
      {showSettingsModal && (
        <div className="modal-overlay" onClick={() => setShowSettingsModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '560px', width: '100%' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '16px' }}>
              정기 모임 요일을 지정하면 향후 8주간의 일정이 자동으로 생성됩니다.
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
              {tempMeetings.map((tm, idx) => (
                <div key={idx} style={{ display: 'flex', gap: '8px', alignItems: 'center', background: 'var(--bg)', padding: '10px', borderRadius: '8px', flexWrap: 'wrap' }}>
                  <select 
                    className="select" 
                    style={{ width: '90px' }} 
                    value={tm.dayOfWeek}
                    onChange={e => {
                      ...
                      next[idx].title = `정기 모임 (${DAYS[next[idx].dayOfWeek]})`;
              ...
              <button 
                ...
                onClick={() => setTempMeetings([...tempMeetings, { dayOfWeek: 2, title: '정기 모임 (화)', startTime: '19:00', endTime: '22:00', location: '테니스장' }])}
              >
                + 정기 모임 요일 추가
              </button>
            </div>
