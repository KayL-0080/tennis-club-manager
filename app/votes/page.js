...
export default function VotesPage() {
  ...
  const saveEdit = async () => {
    ...
    if (selectedEvent) {
      ...
      updates.attendees = {};
      const id = await createEvent(currentClubId, updates);
                                ...
                                return (
                                  <td 
                                    ...
                                    onClick={async () => {
                                      ...
                                      await updateEvent(currentClubId, e.id, { [`settlementAttendees.${member.id}`]: newStatus });
                                    }}
                                  >
                                    {member?.name || ''}
                                  </td>
                              ...
                              return (
                                <td key={`snk-${e.id}`} style={{ border: '1px solid var(--border)', padding: '4px', fontSize: '14px', textAlign: 'right' }}>
                                  <input 
                                    ...
                                    onBlur={(ev) => {
                                      const val = parseInt(ev.target.value) || 0;
                                      updateEvent(currentClubId, e.id, { snackFeePerPerson: val });
                                    }}
                                  />
                                  <br/>
